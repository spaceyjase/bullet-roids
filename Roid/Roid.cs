...
{
  [Signal] public delegate void ExplodedEventHandler(int size, int radius, Vector2 position, Vector2 linearVelocity);
  
  [Export] private int baseHealth = 3;
  ...
  public void Damage()
  {
    if (health < 0) return; // I might already be dead; bullets move fast ;)
